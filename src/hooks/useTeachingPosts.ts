...
const initialMockPosts: TeachingPost[] = [
  {
    ...
    videos: [
      {
        title: "Sensores na Indústria 4.0",
  ...
  },
  {
    ...
    videos: [
      {
        title: "CLP na Prática - Programação Ladder",
  ...
  },
  {
    ...
    videos: [
      {
        title: "IoT na Indústria 4.0 - Conceitos Básicos",
        url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ",
        platform: "youtube",
      },
      {
        title: "Sensores IoT em Ação",
...
export const useTeachingPosts = () => {
  ...
  const fetchPosts = async () => {
    try {
      ...
      if (!supabase || !import.meta.env.VITE_SUPABASE_URL) {
        ...
        if (savedPosts) {
          ...
          localStorage.setItem(
            "teachingPosts",
            JSON.stringify(initialMockPosts),
          );
        }
        return;
      }
      ...
      let query = supabase.from("teaching_posts").select("*");
  ...
  const createPost = async (
    postData: Omit<TeachingPost, "id" | "created_at" | "likes" | "views">,
  ) => {
    try {
  ...
  const updatePost = async (
    id: string,
    updates: Partial<Omit<TeachingPost, "id" | "created_at">>,
  ) => {
    try {
      if (!user) {
        throw new Error("Usuário não autenticado");
