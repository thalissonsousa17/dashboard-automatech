...
function MapAutoFit({ logs }: { logs: AccessLog[] }) {
  const map = useMap();
  useEffect(() => {
    const points = logs
    ...
    <div className="min-h-full bg-gray-950 text-gray-100 rounded-xl overflow-hidden">
      <div className="p-5 md:p-6">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-4">
          <input
            type="text"
            placeholder="Buscar por usuário, país, cidade ou IP..."
            value={searchTerm}
                  ...
                  .map(log => (
                    <CircleMarker
                      ...
                      pathOptions={{
                        ...
                        fillColor: log.status === 'online' ? '#4ade80' : '#f87171',
                        fillOpacity: 0.8,
                        weight: 2,
                      }}
                    >
                      <Popup>
                        <div className="text-gray-900 text-sm min-w-[180px]">
                          <p className="font-bold text-base mb-1">
                            {flagEmoji(log.country_code)} {log.city || '—'}, {log.country || '—'}
                          </p>
                          <p><b>Usuário:</b> {log.user_name || log.user_email || 'Anônimo'}</p>
                          <p><b>IP:</b> {log.ip_address}</p>
                          <p><DeviceIcon type={log.device_type} />{log.browser} / {log.os}</p>
                          <p className={`font-semibold mt-1 ${log.status === 'online' ? 'text-green-600' : 'text-red-500'}`}>
                            {log.status === 'online' ? '● Online' : '○ Offline'}
                          </p>
                          <p className="text-gray-500 text-xs mt-1">Login: {formatDate(log.logged_in_at)}</p>
                        </div>
                      </Popup>
                    </CircleMarker>
                  ))}
            ...
            <div className="bg-gray-900 rounded-xl border border-gray-800 p-4">
              ...
              <div className="space-y-3 overflow-y-auto max-h-96">
                {countryStats.slice(0, 15).map(stat => (
                  <div key={stat.country_code} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs ml-2">
                      {stat.currently_online > 0 && (
                        <span className="text-green-400 font-medium">{stat.currently_online} 🟢</span>
                      )}
                ...
                {countryStats.length === 0 && (
                  <p className="text-gray-500 text-sm text-center py-8">Sem dados ainda</p>
                )}
              </div>
            </div>
          </div>
                  ...
                  {filteredLogs.length === 0 ? (
                    ...
                    filteredLogs.map(log => (
                      <tr key={log.id} className="border-b border-gray-800/50 hover:bg-gray-800/40 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <span className="text-base mr-1">{flagEmoji(log.country_code)}</span>
